...
all_data$response <- toupper(all_data$response)

all_data$accuracy <- ifelse(substring(all_data$stimulus, 2, 2) == all_data$response, "TRUE", "FALSE" ) 
```
...
```{r, eval=TRUE}
all_data$reactiontime <- as.numeric(all_data$response_time - all_data$stimulus_onset)

class(all_data$reactiontime)
```
...
```{r, eval=TRUE}
RTless2000ms <- subset(all_data, all_data$reactiontime < 2000)
```
